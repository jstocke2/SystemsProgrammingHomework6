...
int get_ticket()
{
	...
	sprintf(ipTicket->buf, "HELO %d", pid);		/* compose request	*/

	if ( (response = do_transaction(ipTicket)) == NULL )
		return(-1);
...
int release_ticket()
{
	...
	sprintf(ipTicket1.buf, "GBYE %s", ticket_buf);	/* compose message	*/
	if ( (response = do_transaction(ipTicket1.buf)) == NULL )
		return(-1);
 ...
 */
char* do_transaction(struct ipTicket)
{
	...
	ipTicket ipTicketRecv;
	
	
	ret = sendto(sd, ipTicketRecv, strlen(ipTicketRecv), 0, &serv_addr, serv_alen);
	if ( ret == -1 ){
		syserr("sendto");
		return(NULL);
	}
	...
	ret = recvfrom(sd, ipTicketRecv, MSGLEN, 0, &retaddr, &addrlen);
	if ( ret == -1 ){
		syserr("recvfrom");
		return(NULL);
	}
